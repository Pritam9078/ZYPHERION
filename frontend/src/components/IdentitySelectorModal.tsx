      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
          <motion.div 
            ...
            className="w-full max-w-lg glass border border-slate-200 dark:border-white/10 rounded-[3rem] p-10 relative shadow-2xl overflow-hidden"
          >
            ...
            <button 
              onClick={onClose}
              className="absolute top-8 right-8 text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors z-20"
            >
            ...
            <div className="mb-10 relative z-10 text-center">
              ...
              <h3 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tighter mb-4 uppercase">Select Identity_</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400 max-w-xs mx-auto">Choose your specialized role to initialize the correct protocol telemetry.</p>
            </div>
              ...
              {[
                ...
                >
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <span className="font-bold text-sm block uppercase tracking-tight">{role.label}</span>
                      <span className="text-[10px] opacity-60 font-medium tracking-wider uppercase">{role.desc}</span>
                    </div>
                  </div>
                  {selectedRole === role.id && (
                    <motion.div 
                      layoutId="active-indicator"
                      className="w-2.5 h-2.5 rounded-full bg-blue-600 shadow-[0_0_12px_rgba(37,99,235,0.6)]" 
                    />
                  )}
                </button>
              ))}
            ...
            <button 
              ...
              className="w-full py-6 bg-slate-900 dark:bg-white text-white dark:text-black rounded-3xl font-bold text-xs uppercase tracking-wider transition-all shadow-2xl active:scale-[0.97] border border-transparent dark:border-white/20 italic"
            >
              Initialize_Uplink
            </button>
          </motion.div>
